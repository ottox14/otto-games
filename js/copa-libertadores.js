...
(function(){
  ...
  var WALL_REST = 0.7;
  var BODY_REST = 0.88;
  var MATCH_DURATION = 120; // segundos (2 minutos) - sin límite de tiros, se juega a reloj
  ...
  // defenseChance/ballDeep: que tan seguido despeja aunque no haya peligro real.
  // shotThresholdMult: mas bajo = tira mas seguido (le alcanza con menos angulo libre).
  // aggression: probabilidad de buscar pase de contragolpe en vez de despejar a lo loco.
  // thinkTime: rango de "reflejos" (segundos) antes de jugar su turno.
  var DIFFICULTIES = {
    facil:      {label:'Fácil',      smart:0, aimNoise:0.85,  powerNoise:0.5,  mistakeChance:0.42, defenseChance:0.20, shotThresholdMult:1.35, aggression:0.15, thinkTime:[0.7,1.3]},
    normal:     {label:'Normal',     smart:1, aimNoise:0.26,  powerNoise:0.26, mistakeChance:0.10, defenseChance:0.32, shotThresholdMult:1.0,  aggression:0.45, thinkTime:[0.55,1.0]},
    dificil:    {label:'Difícil',    smart:2, aimNoise:0.12,  powerNoise:0.15, mistakeChance:0.04, defenseChance:0.38, shotThresholdMult:0.78, aggression:0.7,  thinkTime:[0.4,0.75]},
    legendario: {label:'Legendario', smart:3, aimNoise:0.045, powerNoise:0.07, mistakeChance:0.015,defenseChance:0.4,  shotThresholdMult:0.55, aggression:0.95, thinkTime:[0.28,0.5]}
  };
  ...
  var campaign = {
    ...
    rounds: [], roundIndex: 0, stage: null, yourGroupIndex: 0,
    groupQualifiers: [], bracket: null, shootoutNote: false
  };
  ...
  // Fixture de 6 fechas (ida y vuelta) para un grupo de 4, con group[0]
  // siempre tu equipo. Fechas 1-3 (ida): vos-1, 2-3 | vos-2, 1-3 | vos-3, 1-2.
  // Fechas 4-6 (vuelta): mismos cruces con local/visitante invertido.
  function buildRoundRobin(group){
    var rounds = [];
    for (var leg=0; leg<2; leg++){
      for (var i=1; i<=3; i++){
        var restIdx = [1,2,3].filter(function(idx){ return idx !== i; });
        var yourHome = leg===0 ? group[0] : group[i];
        var yourAway = leg===0 ? group[i] : group[0];
        var otherHome = leg===0 ? group[restIdx[0]] : group[restIdx[1]];
        var otherAway = leg===0 ? group[restIdx[1]] : group[restIdx[0]];
        rounds.push({
          yourOpponent: group[i],
          yourHome: yourHome, yourAway: yourAway,
          otherHome: otherHome, otherAway: otherAway,
  ...
  function startCampaign(yourId, difficulty){
    ...
    // Clasificados (1ro y 2do) de los otros 7 grupos, simulados de antemano para
    // poder armar el cuadro completo de octavos apenas termines tu fase de grupos.
    // El tuyo (indice yourGroupIndex) se completa recien cuando termines tus 3 fechas.
  ...
  function renderStandingsTable(){
    ...
    sorted.forEach(function(s, idx){
      ...
      if (campaign.roundIndex >= 6 && idx < 2) cls += 'is-qualified';
  ...
  function renderFixtureList(){
    ...
    campaign.rounds.forEach(function(round, idx){
      ...
      var homeIsYou = round.yourHome === campaign.yourId;
      var yourHomeName = TEAMS_BY_ID[round.yourHome].name;
      var yourAwayName = TEAMS_BY_ID[round.yourAway].name;
      var otherHomeName = TEAMS_BY_ID[round.otherHome].name;
      var otherAwayName = TEAMS_BY_ID[round.otherAway].name;

      var yourLine = document.createElement('div');
      var otherLine = document.createElement('div');

      if (round.played){
        var homeGoals = homeIsYou ? round.yourResult[0] : round.yourResult[1];
        var awayGoals = homeIsYou ? round.yourResult[1] : round.yourResult[0];
        yourLine.className = 'fixture-line is-done';
        yourLine.textContent = '✅ '+yourHomeName+' '+homeGoals+'-'+awayGoals+' '+yourAwayName;
        otherLine.className = 'fixture-line is-done';
        otherLine.textContent = '✅ '+otherHomeName+' '+round.otherResult[0]+'-'+round.otherResult[1]+' '+otherAwayName;
      } else if (idx === campaign.roundIndex){
        yourLine.className = 'fixture-line is-next';
        yourLine.textContent = '▶ '+yourHomeName+' vs '+yourAwayName+' (Próximo partido)';
        otherLine.className = 'fixture-line is-pending';
        otherLine.textContent = otherHomeName+' vs '+otherAwayName+' (Pendiente)';
      } else {
        yourLine.className = 'fixture-line is-pending';
        yourLine.textContent = yourHomeName+' vs '+yourAwayName+' (Pendiente)';
        otherLine.className = 'fixture-line is-pending';
        otherLine.textContent = otherHomeName+' vs '+otherAwayName+' (Pendiente)';
      }
  ...
  var fixtureCurrentEl = document.getElementById('liberFixtureCurrent');
  function renderCurrentFixtureCard(){
    if (!fixtureCurrentEl) return;
    ...
    var matches = [
      {home: round.yourHome, away: round.yourAway},
      {home: round.otherHome, away: round.otherAway}
    ];
    ...
    matches.forEach(function(m){
      html += '<li>'+TEAMS_BY_ID[m.home].name+' vs '+TEAMS_BY_ID[m.away].name+'</li>';
    });
    html += '</ul>';
  ...
  function showStageScreen(){
    ...
    if (campaign.stage === 'group'){
      ...
      renderFixtureList();
      stageTitleEl.textContent = 'Grupo '+campaign.groupLetter+' · Fecha '+(campaign.roundIndex+1)+' de '+campaign.rounds.length;
      stageTextEl.textContent = '';
      stageContinueBtn.textContent = '▶ Jugar partido';
    } else if (campaign.stage === 'knockout'){
      ...
      stageTextEl.textContent = label+' vs '+TEAMS_BY_ID[yourBracketOpponent()].name;
      stageContinueBtn.textContent = '▶ Jugar '+label.toLowerCase();
    }
  }
  ...
  stageContinueBtn.addEventListener('click', function(){
    ...
    if (campaign.stage === 'group'){
      ...
      startMatchVs(yourBracketOpponent());
    }
  });
  ...
  function simulateKnockoutMatch(aId, bId){
    var score = simRealisticScore(aId, bId, false);
    if (score[0] > score[1]) return aId;
    if (score[1] > score[0]) return bId;
    return Math.random() < 0.5 ? aId : bId; // definicion por penales, no se muestra el detalle
  }

  // Cuadro de octavos: cruza el 1ro de cada grupo con el 2do de OTRO grupo
  // (nunca el propio), como en un sorteo real de copa continental.
  ...
  function resolveKnockout(){
    var yourWon;
    if (scoreHome > scoreAway) yourWon = true;
    else if (scoreAway > scoreHome) yourWon = false;
    else {
      campaign.shootoutNote = true;
      yourWon = Math.random() < 0.5;
    }
  ...
  function afterMatchContinue(){
    ...
    if (campaign.stage === 'group'){
      ...
      var otherScore = simRealisticScore(round.otherHome, round.otherAway, true);
      round.otherResult = otherScore;
      applyResult(round.otherHome, round.otherAway, otherScore[0], otherScore[1]);
      round.played = true;
      campaign.roundIndex += 1;
      if (campaign.roundIndex >= campaign.rounds.length) finishGroupStage();
  ...
  function concludeTurn(forceNextTeam){
    ...
    updateMatchHud();
    if (turnTeam === 'away'){
      var tt = currentDifficulty.thinkTime;
      aiThinkTimer = tt[0]+Math.random()*(tt[1]-tt[0]);
    }
  }
  ...
  function performAIMove(){
    ...
    // Error deliberado: mas comun en Facil, raro pero posible incluso en
    // Legendario (para que se sienta humana y no una maquina infalible).
    ...
    var angleSamples = diff.smart >= 3 ? 8 : diff.smart >= 2 ? 6 : diff.smart >= 1 ? 4 : 2;

    // Defensa real: solo despeja a la fuerza si hay peligro genuino cerca de
    // su arco, o (con menor frecuencia) si la pelota esta bien metida en su
    // propio tercio - nunca solo por estar "en su mitad", para que no se
    // pase todo el partido retrocediendo en vez de atacar.
    var dangerNearMyGoal = homePieces.some(function(h){ return dist(h, {x:ownGoalX,y:ownGoalY}) < (PITCH_R-PITCH_L)*0.32; });
    var ballDeepInMyThird = ballBody.x > PITCH_L + (PITCH_R-PITCH_L)*0.66;
    if (dangerNearMyGoal || (ballDeepInMyThird && Math.random() < diff.defenseChance)){
      var defender = awayPieces.slice().sort(function(a,b){ return dist(a,ballBody)-dist(b,ballBody); })[0];
      var dObstacles = homePieces.concat(awayPieces.filter(function(o){ return o !== defender; }));

      // Contragolpe: en vez de despejar a lo loco, busco un companero mas
      // adelantado con via libre para salir jugando rapido hacia el arco rival.
      var outlet = null;
      if (diff.smart >= 1){
        awayPieces.forEach(function(p){
          if (p === defender || p.x > defender.x-20) return;
          var lane = pathClearance(defender.x, defender.y, p.x, p.y, dObstacles);
          if (lane > PIECE_R*0.9 && (!outlet || p.x < outlet.x)) outlet = p;
        });
      }
      if (outlet && Math.random() < diff.aggression){
        fireWithNoise(diff, defender, aimDirectionForTarget(defender, outlet.x, outlet.y), 0.55+Math.random()*0.2);
      } else {
        var clearY = ballBody.y > ownGoalY ? PITCH_T+40 : PITCH_B-40;
        var clearX = PITCH_CX - (PITCH_R-PITCH_L)*(0.05+diff.aggression*0.14) + (Math.random()*50-25);
        fireWithNoise(diff, defender, aimDirectionForTarget(defender, clearX, clearY), 0.72+Math.random()*0.2);
      }
      return;
    }

    // Ataque: elijo la ficha que mejor puede tocar la pelota, evaluo tiro
    // directo, luego pared/pase a un companero mejor ubicado, y si no avanzo
    // la pelota hacia el mejor angulo disponible - nunca me quedo quieta ni
    // retrocedo. Mientras mas dificil, con menos angulo libre ya me alcanza
    // para animarme a rematar.
    ...
    var shotThreshold = PIECE_R*diff.shotThresholdMult;
    ...
    if (canShoot){
      ...
      power = diff.smart >= 2 ? 0.92+Math.random()*0.08 : 0.8+Math.random()*0.15;
    } else if (passOpt && diff.smart >= 1){
      // Pared/pase rapido a un companero mejor posicionado - la base de las
      // jugadas de dos toques que buscan las dificultades mas altas.
      ...
      // angulo que encontre, siempre hacia adelante, para seguir generando
      // peligro en vez de retroceder o quedarme quieta.
      targetPoint = shotOpt.point;
