...
(function(){
  ...
  function makePlayer(team, x, isAI, attackDir){
    return {
      team: team, x: x, y: GROUND_Y-PLAYER_R, vx: 0, vy: 0,
  ...
  function resetKickoff(){
    home.x = CANVAS_W*0.28; home.y = GROUND_Y-PLAYER_R; home.vx=0; home.vy=0; home.onGround=true; home.kickCooldown=0;
    away.x = CANVAS_W*0.72; away.y = GROUND_Y-PLAYER_R; away.vx=0; away.vy=0; away.onGround=true; away.kickCooldown=0;
  ...
  function drawPlayer(p, t){
    ...
    var legSwing = Math.abs(p.vx) > 5 && p.onGround && !reducedMotion ? Math.sin(t*10)*8 : 0;
    c.strokeStyle = p.team.shorts;
    c.lineWidth = 7;
    c.lineCap = 'round';
    if (p.kicking > 0){
      var kProg = 1-(p.kicking/0.18);
      var kickX = p.attackDir*(12+kProg*18);
      c.beginPath(); c.moveTo(-5,PLAYER_R-6); c.lineTo(-5-p.attackDir*3, PLAYER_R+12); c.stroke();
      c.beginPath(); c.moveTo(5,PLAYER_R-8); c.lineTo(kickX, PLAYER_R-4); c.stroke();
    } else {
      c.beginPath(); c.moveTo(-6,PLAYER_R-6); c.lineTo(-6+legSwing*0.35,PLAYER_R+13); c.stroke();
      c.beginPath(); c.moveTo(6,PLAYER_R-6); c.lineTo(6-legSwing*0.35,PLAYER_R+13); c.stroke();
    }
