...
(function(){
  ...
  function nextKick(){
    ...
    kickState = {
      ...
      finalU: null, finalV: null, wide: false, post: false, keeperTimeBonus: 0, touched: false,
  ...
  // El arquero SOLO puede atajar si la pelota pasa dentro de su radio de
  // estirada real (maxReach): mas alla de eso, por mas "suerte" que haya,
  // nunca la toca - nunca mas un "ATAJADON" con la pelota adentro y el
  // arquero lejos. Dentro de ese radio si compite una probabilidad real
  // (dificultad/plantel/potencia siguen pesando), y puede tocarla sin
  // frenarla (deflexion) - la pelota entra igual, pero con otra sensacion.
  var SAVE_REACH = 0.62;
  var TOUCH_REACH = 0.82;
  function resolveSave(shot, keeperU, keeperV, reachQuality, skillBonus){
    if (shot.wide || shot.post) return {saved:false, touched:false};
    var rq = (reachQuality===undefined?1:reachQuality);
    var dist = Math.hypot(shot.u-keeperU, (shot.v-keeperV)*1.15);
    var maxReach = SAVE_REACH * clamp(0.55+rq*0.55, 0.35, 1.15);
    if (dist > maxReach){
      var touchReach = TOUCH_REACH * clamp(0.6+rq*0.5, 0.4, 1.1);
      return {saved:false, touched: dist <= touchReach};
    }
    var chance = clamp(0.9*rq - (dist/maxReach)*0.75 + (skillBonus||0) + shot.keeperTimeBonus, 0, 0.95);
    return {saved: Math.random() < chance, touched:true};
  }
  ...
  function resolveKick(){
    ...
    if (shot.wide) kickState.outcome = 'wide';
      ...
      var saveResult = resolveSave(shot, kickState.gkU, kickState.gkV, 1, keeperSkillBonus);
      kickState.touched = saveResult.touched;
      kickState.outcome = saveResult.saved ? 'save' : 'goal';
    }
  ...
  function beginAIKick(){
    ...
    if (shot.wide) kickState.outcome = 'wide';
      ...
      var saveResult = resolveSave(shot, kickState.gkU, kickState.gkV, kickState.gkReachQuality, 0);
      kickState.touched = saveResult.touched;
      kickState.outcome = saveResult.saved ? 'save' : 'goal';
    }
  ...
  // Dibuja un brazo o una pierna como un miembro con dos tramos de color
  // (piel+media o manga+piel) mas un botin/mano, rotado desde su punto de
  // union (cadera/hombro) - asi el jugador se ve como un cuerpo humano
  // proporcionado (torso, brazos y piernas con grosor real) en vez de un
  // muñeco de lineas finas.
  function drawLimb(c, hx, hy, angle, len, w, trimColor, isLeg){
    c.save();
    c.translate(hx, hy);
    c.rotate(angle);
    if (isLeg){
      c.fillStyle = '#e8c99b';
      c.fillRect(-w/2, 0, w, len*0.6);
      c.fillStyle = trimColor;
      c.fillRect(-w/2, len*0.6, w, len*0.25);
      c.fillStyle = '#1c1c1c';
      c.beginPath();
      if (c.roundRect) c.roundRect(-w*0.62, len*0.82, w*1.25, len*0.2, 2); else c.rect(-w*0.62, len*0.82, w*1.25, len*0.2);
      c.fill();
    } else {
      c.fillStyle = trimColor;
      c.fillRect(-w/2, 0, w, len*0.55);
      c.fillStyle = '#e8c99b';
      c.beginPath(); c.arc(0, len*0.55+w*0.4, w*0.55, 0, Math.PI*2); c.fill();
      c.fillRect(-w/2, len*0.55, w, len*0.4);
    }
    c.restore();
  }
  // Jugador de futbol completo: cabeza, torso con camiseta, pantalon,
  // piernas con medias y botines, y brazos - no un palito. `pose.anim`
  // selecciona la postura (idle/run/kick/dive/celebrate/react) y
  // `pose.t` (0..1) mueve esa postura (ciclo de carrera, swing del
  // pierna al patear, rebote del festejo, etc). `pose.rot`/`pose.stretch`
  // siguen controlando la inclinacion/estirada global (usadas por el
  // arquero al tirarse), igual que antes.
  function drawFigure(x,y,shirt,band,pose){
    var c = ctx;
    var scale = ((pose && pose.scale) || 1) * 1.12;
    var anim = (pose && pose.anim) || 'idle';
    var t = (pose && pose.t !== undefined) ? pose.t : 0;
    var rot = pose && pose.rot ? pose.rot : 0;
    var stretch = pose && pose.stretch ? pose.stretch : 0;

    var legL = 0.08, legR = -0.08, armL = 0.35, armR = -0.35, armLRaise = 0, armRRaise = 0;
    if (anim === 'run'){
      var rc = t*Math.PI*2;
      legL = Math.sin(rc)*0.65; legR = Math.sin(rc+Math.PI)*0.65;
      armL = Math.sin(rc+Math.PI)*0.55; armR = Math.sin(rc)*0.55;
    } else if (anim === 'kick'){
      ...
      armLRaise = 1.35; armRRaise = 1.35;
    } else if (anim === 'react'){
      legL = 0.05; legR = -0.05;
      armLRaise = -0.35; armRRaise = -0.35;
    }

    c.save();
    c.translate(x,y);
    c.fillStyle = 'rgba(0,0,0,0.32)';
    c.beginPath(); c.ellipse(0, 38*scale, 19*scale, 6.5*scale, 0, 0, Math.PI*2); c.fill();
    c.rotate(rot);
    c.scale((1+stretch*0.5)*scale, (1-stretch*0.3)*scale);

    drawLimb(c, -6, 9, legL, 22, 6.6, band, true);
    drawLimb(c, 6, 9, legR, 22, 6.6, band, true);

    c.fillStyle = shirt;
    c.beginPath();
    if (c.roundRect) c.roundRect(-10.5,-9,21,22,6); else c.rect(-10.5,-9,21,22);
    c.fill();
    c.strokeStyle = band; c.lineWidth = 3;
    c.stroke();
    c.fillStyle = band;
    c.beginPath();
    if (c.roundRect) c.roundRect(-10.5,-9,21,5,4); else c.rect(-10.5,-9,21,5);
    c.fill();
    c.fillStyle = band;
    c.globalAlpha = 0.85;
    ...
    drawLimb(c, -11, -6, armL-armLRaise, 16, 5, shirt, false);
    drawLimb(c, 11, -6, armR+armRRaise, 16, 5, shirt, false);

    c.fillStyle = '#f0c39b';
    c.beginPath(); c.arc(0,-18,8,0,Math.PI*2); c.fill();
    c.fillStyle = 'rgba(50,32,18,0.65)';
    c.beginPath(); c.arc(0,-21,8,Math.PI*1.05,Math.PI*1.95); c.fill();
  ...
  // El rematador festeja si convirtio (aunque el arquero la haya tocado) y
  // se lamenta si le atajaron, se fue afuera o pego en el palo.
  function kickerReactionAnim(){
    var revealT = clamp(scene.t/0.5, 0, 1);
  ...
  function drawKickingScene(){
    ...
    if (kickState && scene.phase === 'run'){
      ...
      drawFigure(kx, ky, kickState.kickerTeam.shirt, kickState.kickerTeam.band, {anim:'run', t:(scene.runF*3)%1});
    } else if (kickState && scene.phase === 'shot'){
      drawFigure(SPOT.x, SPOT.y+35, kickState.kickerTeam.shirt, kickState.kickerTeam.band, {anim:'kick', t:clamp(scene.shotF/0.4,0,1)});
    } else if (kickState && scene.phase === 'reveal'){
      var kickerReact = kickerReactionAnim();
      drawFigure(SPOT.x, SPOT.y+35, kickState.kickerTeam.shirt, kickState.kickerTeam.band, kickerReact);
    }
  ...
  function drawGoalkeepingScene(){
    ...
    if (scene.phase === 'run'){
      ...
      drawFigure(kx, ky, kickState.kickerTeam.shirt, kickState.kickerTeam.band, {scale:0.3+0.28*scene.runF, anim:'run', t:(scene.runF*3)%1});
    } else if (scene.phase === 'shot'){
      drawFigure(GK_STRIKE.x, GK_STRIKE.y, kickState.kickerTeam.shirt, kickState.kickerTeam.band, {scale:0.58, anim:'kick', t:clamp(scene.shotF/0.4,0,1)});
    } else if (scene.phase === 'reveal'){
      var rivalReact = kickerReactionAnim();
      rivalReact.scale = 0.58;
      drawFigure(GK_STRIKE.x, GK_STRIKE.y, kickState.kickerTeam.shirt, kickState.kickerTeam.band, rivalReact);
    }
